...
async def handle_callback(callback: CallbackQuery):
    ...
    # [FIX] Download stream correctly
        ...
        await callback.message.answer_document(BufferedInputFile(file_bytes, filename=real_name))
...
async def handle_user_input(message: Message):
    ...
    if state['action'] == "search":
        ...
        service.files().update(fileId=f_data['file_id'], body={'name': message.text}).execute()
        ...
        meta = {'name': message.text, 'mimeType': 'application/vnd.google-apps.folder', 'parents': [state['parent_id']] if state['parent_id'] != "root" else []}
    ...
    # [FIX] Upload stream directly to MediaIoBaseUpload
        ...
        if file_obj:
            ...
            try:
                ...
                file_bytes = file_io.read()
                meta = {'name': filename, 'parents': [state['parent_id']] if state['parent_id'] != "root" else []}
                ...
                if await db.is_backup_enabled(user_id):
                    ...
                    if backup_acc:
                        try:
                            ...
                            backup_media = MediaIoBaseUpload(io.BytesIO(file_bytes), mimetype='application/octet-stream')
